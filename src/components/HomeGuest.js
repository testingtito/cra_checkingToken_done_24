...
const HomeGuest = () => {
  ...
  const ourReducer = (draft, action) => {
    switch (action.type) {
      ...
      case "usernameAfterDelay":
        ...
        // 에러가 있을 때 check할 필요가 없다.
        // 이미 client side에서 uniqueness를 체크했으면 다시 할 피요가 없다.
      ...
      case "emailAfterDelay":
        ...
        if (!/^\S+@\S+$/.test(draft.email.value)) {
          ...
          draft.email.message = "You must provide a vliad email address.";
        }
        // if email is unique
      ...
      case "submitForm":
        if (!draft.username.hasErrors &&
          ...
          // if these are all true, we want to send an axios request to submit the form
          draft.submitCount++;
        }
        return;
      default:
  ...
  // for username
  useEffect(() => {
    if (state.username.value) { // if username is not blank
  ...
  // for email
  useEffect(() => {
    if (state.email.value) { // if email is not blank
      const delay = setTimeout(() => {
  ...
  // for password
  useEffect(() => {
    if (state.password.value) { // if password is not blank
      const delay = setTimeout(() => {
    ...
    if (state.username.checkCount) {
      ...
      async function fetchResults() {
        try {
          ...
          const response =
            await Axios.post('/doesUsernameExist',
              { username: state.username.value },
              { cancelToken: ourRequest.token });

          // 두 번째 argument는 server의 response
    ...
    if (state.email.checkCount) {
      ...
      async function fetchResults() {
        try {
          // 1st argument: path 2nd argument: the data we want to send along 3rd argument: cancel toekn
          const response =
            await Axios.post('/doesEmailExist',
    ...
    if (state.submitCount) {
      ...
      async function fetchResults() {
        try {
          ...
          // automatically let user log in
          globalDispatch({ type: "login", data: response.data }) //contains username, avatar, token
          globalDispatch({ type: "flashMessage", value: "Contrats! Welcome to your new account." })
        } catch (e) {
          console.log("There was a problem or the request was cancelled.");
        }
      }
